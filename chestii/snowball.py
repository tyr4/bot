...
class Snowball(commands.GroupCog, name="snowball"):
    ...
    async def hit(self, interaction: discord.Interaction, user: discord.User) -> None:
        if user.id == 556836294710525952:
            embed = discord.Embed(title=f"Snowball haha idk", color=0x1abc9c)
            embed.add_field(name="stop snowballing me idiot", value=f"ඞ", inline=False)
            await interaction.response.send_message(embed=embed, ephemeral=True)
        # elif user.id == interaction.user.id:
        #     embed = discord.Embed(title=f"Snowball haha idk", color=0x1abc9c)
        #     embed.add_field(name="Are you dumb?", value=f"Hmm imo yes", inline=False)
        #     embed.set_footer(text="Ahmet was here",
        #                      icon_url="https://cdn.discordapp.com/emojis/1106570654544830474.png")
            # await interaction.response.send_message(embed=embed, ephemeral=True)
        elif interaction.user.id not in lista and interaction.user.id not in unlucky:
            result = randint(1, 2)
            ...
            lista.append(interaction.user.id)
            if result == 1:
                unlucky.append(user.id)
                await asyncio.sleep(20)
                unlucky.remove(user.id)
            await asyncio.sleep(40 if result == 1 else 60)
            lista.remove(interaction.user.id)
        else:
            embed = discord.Embed(title=f"Snowball haha idk", color=0x1abc9c)
            embed.add_field(name="", value=f"{'You got hit idiot' if interaction.user.id in unlucky else 'Youre on cooldown idiot'}")
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(Snowball(bot), guild=discord.Object(id=748126143584141332))
