...
letters = ['j', 'a', 'c', 'k', 'p', 'o', 't', 'e']
funni_list = []
leaderboard_spots = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
has_talked = []
...
def leaderboard(user: int):
    with open('jackpo.json', 'r') as json_file:
        ...
        for i, (key, value) in enumerate(sorted_data):
            ...
            letters = f"{value['j']} {value['a']} {value['c']} {value['k']} {value['p']} {value['o']} {value['t']} {value['e']}"
            if key == str(user):
                ceva = f"{letters} - ({value['sum']}) {value['name']}"
                if i >= 10:
                    break
            if i < 10 and value['sum'] != 0:
                idk += f"{letters} - ({value['sum']}) {value['name']}\n"

        idk += "———————————————\nJ A C K P O T E"
        embed.add_field(name="**TOP 10**", value=idk + "```", inline=False)
        if ceva:
            embed.add_field(name="You\n\n", value="```J A C K P O T E\n———————————————\n" + ceva + "```", inline=False)
        else:
...
def update(user_id: str, name, letter, mode):
    with open('jackpo.json', 'r+') as json_file:
        ...
        if user_id not in data:
            data[user_id] = {"name": name, "trade_state": False, "j": 0, "a": 0, "c": 0, "k": 0, "p": 0, "o": 0, "t": 0, "e": 0, "sum": 0}
    ...
    async def on_message(self, message):
        ...
        if message.guild.id == 1134464290477330432:
            ...
            if message.author.bot is True:
                print(f"{message.author.display_name} e bot fraiere")
                return
            ...
            if letter_chance == 1_000:
                ...
                if len(str(message.content)) < 5 and len(str(message.content)) != 0:
                    ...
                    await message.reply(f'WE HAVE A WINNER!!!!!\n'
                                        f'You won the letter **{letter_won.upper()}**!\nCongrats my friend {message.author.display_name}! <:pepeflower:1134466332113506364>', mention_author=True)
            ...
            if jackpo_chance == 100_000:
                if len(str(message.content)) < 5 and len(str(message.content)) != 0:
                    ...
                    await message.reply(f'WE HAVE A JACKPO WINNER!!!!!!!!!!!!!!!!\n'
                                        f'{message.author.display_name} WON JACKPO!!!!!!!!!!!!!!\nCONGRATS {message.author.display_name}! {"<:LETSFUCKINGGOO:1134465653932314666>" * 20}', mention_author=True)
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(Jackpo(bot), guild=discord.Object(id=993818190008287283))