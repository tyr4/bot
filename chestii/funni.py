...
def update(name: str, mode):
    with open('data.json', 'r+') as json_file:
        data = json.load(json_file)
        if name not in data:
            data[name] = {"kurureact1": 0, "kurureact2": 0, "kuruemote": 0, "kurugif": 0}

        if mode == 1:
            data[name]['kurureact1'] += 1
        elif mode == 2:
            data[name]['kurureact2'] += 1
        elif mode == 3:
            data[name]['kuruemote'] += 1
        elif mode == 4:
            data[name]['kurugif'] += 1
...
class Funni(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.guild.id in [570929677732937738, 993818190008287283]:
            ...
            if zplus == 1000 or str(message.channel) == "amogus":
