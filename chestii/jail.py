...
class Jail(commands.GroupCog, name="jail"):
    ...
    async def jailadd(self, interaction: discord.Interaction, user: discord.Member):
        if user.id in [556836294710525952, 1135983715646976111]:
            ...
            await interaction.response.send_message(f"Added `{user.name}` to the jail list! <:PeepoNote:1134465659305209936>")
        else:
    ...
    async def blackjack(self, interaction: discord.Interaction):
        ...
        if not in_game:
            if interaction.user.id in ceva_id:
                if interaction.channel.id == 1287481287300616212:
                    view = InitialView()

                    embed.add_field(name="You made it to Gulag!", value=f"Get over **6969** points in order "
                    ...
                    embed.add_field(name="#jackblack idiot", value="", inline=False)
    ...
    async def stopbet(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if get_value(interaction.user.id, 'bet', interaction.user.name) > 0:
            ...
            if not rules(player_cards, dealer_cards, suma(player_cards, hidden_card), suma(dealer_cards, hidden_card),
                ...
                if get_value(interaction.user.id, 'score', interaction.user.name) < 6969:
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(Jail(bot), guild=discord.Object(id=1134464290477330432))
