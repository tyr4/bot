...
class Funni(commands.Cog):
    ...
    async def on_message(self, message):
        channel = self.client.get_channel(1166774967552184380)
        ...
        if message.author.id == 1007925941093281812:
            await channel.send(f"**{name} in <#{message.channel.id}>**: {message.content}")
            await asyncio.sleep(5)
            await message.delete()
            await message.channel.send(":)", ephemeral=True)
...
async def setup(client):
    await client.add_cog(Funni(client))